...
FILES:${PN}:remove_mender-install = " /boot/EFI/BOOT/${GRUB_IMAGE} "
FILES:${PN}:append_mender-install = " ${MENDER_BOOT_PART_MOUNT_LOCATION}/EFI/BOOT/${GRUB_IMAGE} "

# When using mender and efi-secure-boot, these conf files will be provided by grub-mender-grubenv
CONFFILES:${PN}:remove_mender-grub = " \
...
python do_cleanconfigs:class-target() {
